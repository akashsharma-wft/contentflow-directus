...
export function CtaSection({ section, translationId, translationRow }: Props) {
  const {
    primaryButton,
    secondaryButton,
    theme = 'indigo',
  ...
  const heading = translationRow?.cta_heading ?? section.heading
  const body    = translationRow?.cta_body    ?? section.body

  // CTA labels/hrefs: prefer translationRow fields, fall back to section config
  const primaryLabel = translationRow?.cta_primary_label ?? primaryButton?.label
  const primaryHref  = translationRow?.cta_primary_href  ?? primaryButton?.href
  ...
  return (
    ...
    >
      ...
      >
        <h2
          data-directus={pageAttr(translationId, 'cta_heading')}
        ...
        {body && (
          <p
            data-directus={pageAttr(translationId, 'cta_body')}
        ...
        {(primaryLabel || secondaryButton?.label) && (
          <div className={cn('flex gap-4 flex-wrap', centered ? 'justify-center' : '')}>
            {primaryLabel && primaryHref && (
              <Link
                href={primaryHref}
                data-directus={pageAttr(translationId, 'cta_primary_label,cta_primary_href')}
